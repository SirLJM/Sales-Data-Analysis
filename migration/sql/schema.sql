...
(
    model      VARCHAR(5) PRIMARY KEY,
    grupa      VARCHAR(50)  NOT NULL,
    podgrupa   VARCHAR(50)  NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_category_mappings_podgrupa ON category_mappings (podgrupa);
CREATE INDEX IF NOT EXISTS idx_category_mappings_kategoria ON category_mappings (kategoria);
CREATE INDEX IF NOT EXISTS idx_category_mappings_grupa ON category_mappings (grupa);
CREATE INDEX IF NOT EXISTS idx_category_hierarchy ON category_mappings (podgrupa, kategoria);

COMMENT ON TABLE category_mappings IS 'Clothing category hierarchy mapping for products';
COMMENT ON COLUMN category_mappings.grupa IS 'Top-level group: Dzieci, Dorośli, Niemowlę';
COMMENT ON COLUMN category_mappings.podgrupa IS 'Age group: Dziecko, Junior, Kobieta, Mężczyzna, Niemowlę';
COMMENT ON COLUMN category_mappings.kategoria IS 'Clothing type: Bluzy, Spodnie, Koszulki i bluzki, etc.';
