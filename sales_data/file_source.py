...
class FileSource(DataSource):
    ...
    def get_order_priorities(
        ...
        forecast_date = forecast_df['data'].max() if 'data' in forecast_df.columns else datetime.now()
    ...
    def get_monthly_aggregations(
        ...
        monthly_agg = monthly_data.groupby([group_col, 'year_month'], as_index=False).agg({
            'ilosc': 'sum',
            'razem': 'sum',
            'order_id': 'nunique'
        }).rename(columns={
